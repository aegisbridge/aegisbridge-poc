// scripts/admin_pause_target.js
//
// Pause TargetBridge di Amoy (hanya owner).
...
async function main() {
  ...
  console.log("Calling pause() on TargetBridge...");
  const tx = await bridge.pause();
  console.log("tx hash:", tx.hash);
  const rc = await tx.wait();
  console.log("confirmed in block:", rc.blockNumber);
  console.log("Done. TargetBridge is now PAUSED.");
}

main().catch((err) => {
  console.error("admin_pause_target error:", err);
  process.exit(1);
});
