// scripts/admin_unpause_source.js
//
// Unpause SourceBridge di Sepolia (hanya owner).
...
async function main() {
  ...
  console.log("Calling unpause() on SourceBridge...");
  const tx = await bridge.unpause();
  console.log("tx hash:", tx.hash);
  const rc = await tx.wait();
  console.log("confirmed in block:", rc.blockNumber);
  console.log("Done. SourceBridge is now UNPAUSED.");
}

main().catch((err) => {
  console.error("admin_unpause_source error:", err);
  process.exit(1);
});
